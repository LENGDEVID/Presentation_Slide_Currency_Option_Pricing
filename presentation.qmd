...
def gk_greeks(S, K, rd, rf, sigma, tau, opt='call'):
    # Handle the boundary case (at expiry)
    if tau <= 1e-10:
        delta = 1.0 if S > K and opt=='call' else (-1.0 if S < K and opt=='put' else 0.0)
        return {'delta': delta, 'gamma': 0, 'vega': 0, 'theta': 0}
    
    # Standard Garman-Kohlhagen d1 calculation
    d1 = (np.log(S / K) + (rd - rf + 0.5 * sigma**2) * tau) / (sigma * np.sqrt(tau))
    
    if opt == 'call':
        # Delta for FX Call: e^(-rf * tau) * N(d1)
        delta = np.exp(-rf * tau) * norm.cdf(d1)
    else:
        # Delta for FX Put: e^(-rf * tau) * (N(d1) - 1)
        delta = np.exp(-rf * tau) * (norm.cdf(d1) - 1)
        
    return {'delta': delta} # Add gamma, vega, theta calculations here as needed
