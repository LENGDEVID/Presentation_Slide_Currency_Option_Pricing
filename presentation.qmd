...
- **90% risk reduction**: Unhedged P&L swings wildly with spot moves, but delta-hedged position stays nearly flat
- **Left plot shows effectiveness**: Red (unhedged) has large swings, green (hedged) stays close to zero → hedging works!
- **Right plot quantifies risk**: Volatility drops dramatically from ~$3-4 to ~$0.3-0.4 → much safer position
- **Residual 10% risk comes from**: Gamma (Delta changes between rebalances), Vega (volatility shifts), and discrete rebalancing
...
# 1. Analytical Solution (Garman-Kohlhagen)
def gk_call(S, K, rd, rf, sigma, tau):
    if tau <= 0: return np.maximum(S - K, 0)
    d1 = (np.log(S/K) + (rd - rf + 0.5*sigma**2)*tau) / (sigma*np.sqrt(tau))
    d2 = d1 - sigma*np.sqrt(tau)
    return S*np.exp(-rf*tau)*norm.cdf(d1) - K*np.exp(-rd*tau)*norm.cdf(d2)
...
def btcs(S, K, rd, rf, sigma, T, M, N):
    ...
    for n in range(N-1, -1, -1):
        ...
        V[n,1:M] = thomas1(a[1:], b, c[:-1], d)
    return V

# 3. Corrected Crank-Nicolson Function
def crank_nicolson(S, K, rd, rf, sigma, T, M, N):
    """Crank-Nicolson scheme for Garman-Kohlhagen PDE"""
    # Ensure S is an array to avoid IndexError
    dS = S[1] - S[0]
    dt = T / N
    V = np.zeros((N + 1, M + 1))
    
    # Initial Condition at Expiry (T)
    V[N, :] = np.maximum(S - K, 0)
    
    # Boundary Conditions for all time steps
    tau_grid = T - np.linspace(0, T, N + 1)
    V[:, 0] = 0 # S -> 0
    # Corrected Boundary for FX: include exp(-rf * tau)
    V[:, M] = S[M] * np.exp(-rf * tau_grid) - K * np.exp(-rd * tau_grid)
    ...
    for n in range(N - 1, -1, -1):
        ...
        for i in range(1, M):
            ...
            # Populate LHS and RHS diagonals
            if idx > 0: 
                a_lhs[idx], a_rhs[idx] = -alpha, alpha
            if idx < M - 2: 
                c_lhs[idx], c_rhs[idx] = -gamma, gamma
            b_lhs[idx], b_rhs[idx] = 1 + beta, 1 - beta
            
            # Construct RHS vector using values from step n+1
            rhs[idx] = a_rhs[idx] * V[n+1, i-1] + b_rhs[idx] * V[n+1, i] + c_rhs[idx] * V[n+1, i+1]
        
        # Adjust RHS for boundaries at step n and n+1
        alpha_first = 0.25 * dt * ((sigma * S[1])**2 / dS**2 - (rd - rf) * S[1] / dS)
        gamma_last  = 0.25 * dt * ((sigma * S[M-1])**2 / dS**2 + (rd - rf) * S[M-1] / dS)
        
        rhs[0] += alpha_first * (V[n, 0] + V[n+1, 0])
        rhs[-1] += gamma_last * (V[n, M] + V[n+1, M])
        
        # Solve tridiagonal system for the interior
        V[n, 1:M] = thomas1(a_lhs[1:], b_lhs, c_lhs[:-1], rhs)
...
S0, K, T, sigma = 100, 100, 1.0, 0.2
rd, rf = 0.05, 0.03
S_min, S_max, M, N = 0, 200, 100, 100

# Explicitly create S as an array to fix the Scalar Error
S = np.linspace(S_min, S_max, M + 1)
...
print(f"Price at S={S0}: CN={V_cn[0, i_test]:.6f}, Exact={V_exact:.6f}")
print(f"Relative Error: {abs(V_cn[0, i_test] - V_exact)/V_exact*100:.4f}%")
